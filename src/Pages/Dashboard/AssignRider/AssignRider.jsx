...
const AssignRider = () => {
    ...
    const { data: riders = [], isLoading: isRidersLoading } = useQuery({
        ...
        queryFn: async () => {
            const res = await axiosSecure.get(`/users/riders/${selectedParcel.receiverDistrict}`);
            return res.data;
        }
    });
            ...
            <div className="max-w-[100%] mx-auto">
                ...
                <div className="flex items-center justify-between mb-6 bg-white p-4 rounded-2xl shadow-sm border border-slate-100">
                    <div className="flex items-center gap-3">
                        ...
                        <h2 className="text-2xl font-black text-slate-800 uppercase italic tracking-tighter">Assign <span className="text-orange-600">Riders</span></h2>
                    </div>
                ...
                {parcels.length === 0 ? (
                    <div className="bg-white p-16 rounded-[2rem] text-center border-2 border-dashed border-slate-200"><Package size={40} className="mx-auto text-slate-200 mb-4" /><p className="text-slate-400 font-bold uppercase text-xs">No parcels to assign</p></div>
                ) : (
                            ...
                            <tbody className="divide-y divide-slate-100 text-[13px]">
                                {parcels.map((parcel) => (
                                    <tr key={parcel._id} className="hover:bg-slate-50 transition-all">
                                        <td className="px-4 py-3"><p className="font-bold text-slate-800 leading-none">{parcel.senderName}</p><p className="text-[11px] text-slate-400 mt-1">{parcel.senderEmail}</p></td>
                                        ...
                                        <td className="px-4 py-3 text-slate-500 whitespace-nowrap font-medium">{parcel.creationDate?.split(',')[0]}</td>
                                        <td className="px-4 py-3 font-black text-slate-800">${parcel.deliveryCost}</td>
                                        <td className="px-4 py-3"><span className="px-2 py-0.5 rounded-md bg-green-100 text-green-600 text-[9px] font-black uppercase tracking-tighter">{parcel.paymentStatus}</span></td>
                ...
                <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                    <div className="bg-white w-full max-w-md rounded-[2rem] shadow-2xl overflow-hidden animate-in zoom-in duration-200">
                        <div className="bg-slate-900 p-5 text-white flex justify-between items-center">
                            ...
                            <button onClick={() => setSelectedParcel(null)}><X size={20}/></button>
                        </div>
                        <form onSubmit={handleAssign} className="p-6 space-y-4">
                            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Assigning for: <span className="text-orange-600">{selectedParcel.receiverDistrict}</span></p>
                            
                            <select required className="w-full bg-slate-50 border p-3 rounded-xl font-bold text-sm outline-none" onChange={(e) => setSelectedRider(e.target.value)} defaultValue="">
                                <option value="" disabled>Choose a Rider...</option>
                                {riders.map(rider => (
                                    <option key={rider._id} value={rider._id}>{rider.name} ({rider.email})</option>
                                ))}
                            </select>
                            {riders.length === 0 && !isRidersLoading && <p className="text-red-500 text-[10px] font-bold">* No riders in this district!</p>}

                            <input required type="date" className="w-full bg-slate-50 border p-3 rounded-xl font-bold text-sm outline-none" onChange={(e) => setDeliveryDate(e.target.value)} />

                            <button disabled={riders.length === 0 || assignMutation.isPending} className="w-full bg-slate-900 text-white py-3 rounded-xl font-black uppercase text-xs tracking-widest hover:bg-orange-600 transition-all disabled:bg-slate-200">
